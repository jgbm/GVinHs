...
instance (LLC repr, P.GV os is repr, Conv repr) => GV (STP os is) (RP os is repr) where
  ...
  chooseLeft (RP (m :: (P.GV os is repr, Conv repr) => repr tf i o (STP os is (s1 <++> s2)))) =
    RP (otosShift (polarity :: SPolarity s1) (P.chooseLeft (stoo m)))
  chooseRight (RP (m :: (P.GV os is repr, Conv repr) => repr tf i o (STP os is (s1 <++> s2)))) =
